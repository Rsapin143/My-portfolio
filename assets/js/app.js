...
(() => {
  ...
  const supa = (hasSupabaseConfig && window.supabase)
  ...
  const DEFAULT_WORKS = [
    {
        "id": 1,
        "title": "Freelance Video Editing Portfolio",
        "description": "A collection of short-form edits, cinematic cuts, social media videos, and promotional content focused on pacing, clean transitions, storytelling, captions, and engaging visual flow.",
        "tech": [
            "CapCut",
            "Premiere Pro",
            "After Effects",
        ...
        "images": [
            "https://images.unsplash.com/photo-1574717024653-61fd2cf4d44d?w=900&h=600&fit=crop",
            "https://images.unsplash.com/photo-1536240478700-b869070f9279?w=900&h=600&fit=crop"
        ],
    ...
    {
        "id": 2,
        "title": "Brand Graphics & Social Media Designs",
        "description": "A graphic design showcase featuring social media layouts, posters, thumbnails, banners, and visual branding concepts made for clean communication and strong audience attention.",
        "tech": [
            "Photoshop",
            "Canva",
            "Branding",
    ...
    },
    {
        ...
        "description": "A personal portfolio website built with responsive layouts, interactive sections, reusable components, smooth animations, admin editing, and real-time content updates through Supabase.",
        "tech": [
            "HTML",
            "CSS",
            "JavaScript",
        ...
        "images": [
            "https://images.unsplash.com/photo-1467232004584-a241de8bcf5d?w=900&h=600&fit=crop",
    ...
    {
        "id": 4,
        "title": "Front-End UI Dashboard Concept",
        "description": "A modern dashboard interface concept focused on organized data, clear hierarchy, smooth user experience, form controls, responsive cards, and clean UI behavior.",
        "tech": [
            "React.js",
            "TypeScript",
            "Tailwind CSS",
    ...
    {
        "id": 5,
        "title": "Mobile App Interface Practice",
        "description": "A mobile UI practice project exploring clean screens, simple navigation, responsive components, and app-style interfaces using cross-platform tools and backend services.",
        "tech": [
            "React Native",
            "Flutter",
            "Firebase",
  ...
  async function loadRemoteData() {
    ...
    if (error && error.code !== 'PGRST116') {
      ...
      showFlash('Supabase error: ' + message, 'error');
      return;
    }
  ...
  function getTheme() {
    return (state.profile && state.profile.theme === 'light') ? 'light' : 'dark';
  }
  ...
  function toggleTheme() {
    state.profile.theme = getTheme() === 'light' ? 'dark' : 'light';
    applyTheme();
    saveData();
    showFlash(`${state.profile.theme === 'light' ? 'Light' : 'Dark'} mode enabled.`);
    render();
  }
  ...
  function icon(name, size = 17, strokeWidth = 1.8) {
    ...
    const icons = {
      ...
      sun: `<circle ${c} cx="12" cy="12" r="4" /><path ${c} d="M12 2v2" /><path ${c} d="M12 20v2" /><path ${c} d="m4.93 4.93 1.41 1.41" /><path ${c} d="m17.66 17.66 1.41 1.41" /><path ${c} d="M2 12h2" /><path ${c} d="M20 12h2" /><path ${c} d="m6.34 17.66-1.41 1.41" /><path ${c} d="m19.07 4.93-1.41 1.41" />`,
      moon: `<path ${c} d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z" />`,
    };
      ...
      <div class="admin-page">
        <aside class="admin-sidebar">
          ...
          <div class="sidebar-bottom">
            <button class="sidebar-item" type="button" data-theme-toggle>${getTheme() === 'light' ? icon('moon', 17) : icon('sun', 17)}<span class="sidebar-label">${getTheme() === 'light' ? 'Dark Mode' : 'Light Mode'}</span></button>
    ...
    return `
      ...
      <form id="profileForm">
        <div class="form-grid-2">
          ...
          <div><label class="label">School</label><input class="input-field" name="school" value="${h(p.school || '')}" placeholder="School name"></div>
          <div><label class="label">Course</label><input class="input-field" name="course" value="${h(p.course || '')}" placeholder="Course"></div>
          <div>
            <label class="label">Website Theme</label>
            <select class="input-field" name="theme">
              <option value="dark" ${getTheme() === 'dark' ? 'selected' : ''}>Dark Mode</option>
              <option value="light" ${getTheme() === 'light' ? 'selected' : ''}>Light Mode</option>
            </select>
            <div class="helper-text">This setting is saved in Supabase and updates across devices.</div>
          </div>
          <div>
  ...
  function renderPortfolio() {
    ...
    const roles = [p.title || 'Freelance Video Editor', 'Freelance Video Editor', 'Graphic Designer', 'Front-End Developer', 'UI/UX Designer'];
    return `
      <div style="background:var(--bg);min-height:100vh;">
        ...
        <nav class="nav">
          ...
          <div class="nav-menu">
            ...
            <button class="btn-ghost theme-toggle" type="button" data-theme-toggle>${getTheme() === 'light' ? `${icon('moon', 14)} Dark` : `${icon('sun', 14)} Light`}</button>
          </div>
        </nav>
        <section class="hero">
            ...
            <div class="grid-2">
              ...
              <div>
                ...
                <div class="info-cards">
                  ${p.school ? `<div class="info-card"><span>School</span><strong>${h(p.school)}</strong></div>` : ''}
                  ${p.course ? `<div class="info-card"><span>Course</span><strong>${h(p.course)}</strong></div>` : ''}
                  <div class="info-card"><span>Experience</span><strong>1 Year Freelance Video Editing</strong></div>
                </div>
              </div>
            </div>
          </div>
        ...
        <section id="contact" class="section contact-section reveal">
          <div class="container">
            ...
            <p class="contact-copy">Need a video edit, graphic design, responsive website, or clean UI/UX layout? I am open to freelance projects and collaborations.</p>
  ...
  function initProfileForm() {
    document.getElementById('profileForm')?.addEventListener('submit', async (e) => {
      ...
      state.profile = {
        ...
        theme: form.theme?.value || getTheme(),
        profileImage,
  ...
  async function boot() {
    ...
    initParticles();
    if (!window.supabase) {
      showFlash('Supabase library did not load. Check the CDN script in index.html.', 'error');
    } else if (!hasSupabaseConfig) {
      showFlash('Supabase config is missing. Edit assets/js/supabase-config.js.', 'error');
    }
